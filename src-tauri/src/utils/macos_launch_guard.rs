...
fn move_and_relaunch(bundle: &Path, home: &Path) -> LaunchDisposition {
    ...
    // Always claim the fork's own application path, even when the downloaded bundle was renamed
    // by a browser or user. Reusing the source filename could replace an upstream Clash Verge app.
    let destination = destination_root.join("Clash Verge Next.app");
...
fn sibling_swap_path(destination: &Path, label: &str) -> PathBuf {
    let name = destination
        ...
        .unwrap_or("Clash Verge Next.app");
...
mod tests {
    ...
    fn executable(bundle: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
        let executable = bundle.join("Contents/MacOS/clash-verge-next");
    ...
    fn canonical_allowed_roots_and_nested_bundles_are_accepted() -> anyhow::Result<()> {
        ...
        let system_exe = executable(&system.join("Tools/Clash Verge Next.app"))?;
        let user_exe = executable(&user.join("Network/Clash Verge Next.app"))?;
    ...
    fn failed_staged_activation_preserves_existing_application() -> anyhow::Result<()> {
        ...
        let destination = root.join("Clash Verge Next.app");
        let staging = root.join(".Clash Verge Next.app.installing");
        let backup = root.join(".Clash Verge Next.app.backup");
    ...
    fn canonicalization_accepts_symlink_in_and_rejects_symlink_escape() -> anyhow::Result<()> {
        ...
        let allowed_bundle = system.join("Clash Verge Next.app");
        let allowed_exe = executable(&allowed_bundle)?;
        let link_in = downloads.join("Clash Verge Next.app");
        ...
        assert!(matches!(
            evaluate_install_location_with_roots(&link_in.join("Contents/MacOS/clash-verge-next"), &home, &system),
            LaunchLocation::Allowed { .. }
        ));
        assert!(matches!(
            evaluate_install_location_with_roots(&link_out.join("Contents/MacOS/clash-verge-next"), &home, &system),
            LaunchLocation::Movable { .. }
        ));
    ...
    fn allowed_root_symlink_escape_is_not_authorized() -> anyhow::Result<()> {
        ...
        let escaped_exe = executable(&home.join("Applications/Clash Verge Next.app"))?;
    ...
    fn translocation_and_missing_bundle_are_rejected_before_side_effects() {
        let translocated = std::path::Path::new(
            "/private/var/folders/AppTranslocation/Clash Verge Next.app/Contents/MacOS/clash-verge-next",
        );
        assert_eq!(
        ...
        assert!(matches!(
            evaluate_install_location_with_roots(
                std::path::Path::new("/Users/test/Downloads/clash-verge-next"),
                std::path::Path::new("/Users/test"),
                std::path::Path::new("/Applications")
            ),
